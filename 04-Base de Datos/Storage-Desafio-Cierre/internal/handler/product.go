...
type ProductTopMostSoldJSON struct {
	Description string  `json:"description"`
	Total       float64 `json:"total"`
}

func (h *ProductsDefault) GetTop5MostSoldProducts() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		p, err := h.sv.FindTop5MostSoldProducts()
		if err != nil {
			response.Error(w, http.StatusInternalServerError, "internal server error")
			return
		}

		var data []ProductTopMostSoldJSON
		for _, v := range p {
			data = append(data, ProductTopMostSoldJSON{
				Description: v.Description,
				Total:       v.Total,
			})
		}
